...
type PaymentContext = {
  status: 'payable' | 'paid' | 'no_payable_application' | 'manual_review_required' | 'complimentary_pending' | 'complimentary_active' | 'application_not_found' | 'application_rejected';
...
export default function LawyerPayPage() {
  ...
  if (context?.status === 'complimentary_pending') return shell(<><CheckCircle className="mx-auto mb-4 h-12 w-12 text-amber-500" /><h1 className="mb-2 text-xl font-bold">特邀申请已提交</h1><p className="text-sm text-muted-foreground">管理员审核通过后，特邀资格会自动开通。此申请无需付款。</p></>);
  if (context?.status === 'complimentary_active') return shell(<><CheckCircle className="mx-auto mb-4 h-12 w-12 text-green-600" /><h1 className="mb-2 text-xl font-bold">特邀资格已开通</h1><p className="text-sm text-muted-foreground">您已获得律师特邀资格{context.expiresAt ? `，有效期至 ${new Date(context.expiresAt).toLocaleDateString('zh-CN')}` : ''}。</p><div className="mt-6 flex gap-3"><Link href="/lawyer" className="flex-1 rounded-xl bg-[#C47353] px-4 py-3 font-semibold text-white">进入工作台</Link><Link href="/lawyer/renew" className="flex-1 rounded-xl border px-4 py-3 font-semibold">立即续费</Link></div></>);
