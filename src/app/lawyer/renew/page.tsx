...
import { ArrowLeft, Calendar, CheckCircle, Clock, CreditCard, Loader2, AlertCircle, Smartphone } from 'lucide-react';
...
const renewalPackages = [
  {
    ...
    description: '适合短期试用或刚入驻平台，快速体验接单流程。',
  },
  {
    id: 'civil_renew_18',
    name: '民事律师续费（18个月）',
    ...
    description: '性价比之选，平均每月仅 ¥277，比6个月套餐便宜约44%，适合长期执业。',
    recommended: true,
  },
  {
    id: 'criminal_renew_6',
  ...
  },
  {
    ...
    description: '性价比之选，平均每月仅 ¥444，比6个月套餐便宜约44%，适合长期执业。',
    recommended: true,
  },
];
...
function RenewContent() {
  ...
  // 微信内调起 JSAPI 支付
  const invokeWechatPay = useCallback((params: JsapiPayParams) => {
    const doPay = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#FAF7F2]">
        <div className="bg-white rounded-xl p-8 max-w-sm w-full text-center">
          ...
          <h2 className="text-xl font-serif text-[#3D322D] mb-2">续费成功</h2>
          <p className="text-sm text-[#8C7B6E] mb-6">您的会员已成功续期，页面即将跳转...</p>
          <Link href="/lawyer">
  ...
  if (payParams && !isWechat) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4 bg-[#FAF7F2]">
        <div className="bg-white rounded-xl p-8 max-w-sm w-full text-center">
          <h2 className="text-xl font-serif text-[#3D322D] mb-2">请在微信内支付</h2>
          <p className="text-sm text-[#8C7B6E] mb-6">
            请使用微信扫描下方二维码，在微信内完成支付
          </p>

          <div className="mb-6">
            <div className="inline-block p-4 bg-white rounded-xl border border-[rgba(196,115,83,0.2)]">
              <QRCodeSVG
            ...
            <p className="text-xs text-[#8C7B6E] mt-3">
              {selectedPackage ? `¥${renewalPackages.find(p => p.id === selectedPackage)?.priceDisplay}` : ''}
            </p>
          </div>
          ...
          <div className="flex gap-3">
            <Button
              ...
              className="flex-1 py-3 border-[rgba(196,115,83,0.3)] text-[#8C7B6E] hover:bg-[#FAF7F2] rounded-full font-serif"
            >
              返回选择
            </Button>
            <Button
      ...
      <div className="sticky top-0 z-40 bg-[#FAF7F2]/80 backdrop-blur-md border-b border-[rgba(196,115,83,0.15)]">
        <div className="container mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            <Link
              href="/lawyer"
              className="flex items-center gap-1.5 text-[#C47353] hover:text-[#A85D40] transition-colors duration-150"
            >
              <ArrowLeft className="w-5 h-5" />
              <span className="text-sm font-medium font-serif">返回</span>
            </Link>
            <div className="flex items-center gap-2">
              <span className="text-base font-serif font-normal text-[#C47353]">续费会员</span>
            </div>
            <div className="w-16" />
          </div>
        </div>
      ...
      <div className="container mx-auto px-4 py-6">
        ...
        <Card className="mb-6 border-[rgba(196,115,83,0.2)] bg-[#FAF7F2] shadow-none rounded-xl">
          <CardContent className="pt-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-serif font-normal text-[#3D322D]">续费说明</p>
                <ul className="text-sm text-[#8C7B6E] mt-2 space-y-1">
                  <li>• 续费后会员有效期将在当前到期日基础上顺延</li>
                  <li>• 续费金额按所选套餐时长计算</li>
                  <li>• 18个月套餐平均每月费用更低，比6个月套餐便宜约44%</li>
        ...
        <div className="grid gap-4">
          {renewalPackages.map((pkg) => (
            <Card
              key={pkg.id}
              className={`cursor-pointer transition-all duration-250 shadow-none rounded-xl hover:-translate-y-[2px] ${
                selectedPackage === pkg.id
                  ? 'border-[#C47353] ring-2 ring-[#C47353]/20'
                  : 'border-[rgba(196,115,83,0.2)] hover:shadow-[0_4px_16px_rgba(61,50,45,0.08)]'
              }`}
              ...
              <CardContent className="pt-6">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="font-serif font-normal text-[#3D322D]">{pkg.name}</h4>
                        {pkg.recommended && (
                          <span className="px-2.5 py-0.5 bg-[#FAF7F2] text-[#C47353] text-xs font-serif rounded-full">
                            推荐
                          </span>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-3">
                        {pkg.features.map((feature, index) => (
                          <span
                            key={index}
                            className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#FAF7F2] text-[#8C7B6E] text-xs font-serif rounded-full"
                          >
                            <CheckCircle className="w-3 h-3 text-[#C47353]" />
                            {feature}
                          </span>
                        ))}
                      </div>
                      {/* 🔍 套餐说明 */}
                      {pkg.description && (
                        <p className="text-xs text-[#8C7B6E] mt-3 leading-relaxed">
                          {pkg.description}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="text-right flex-shrink-0 ml-4">
                    <p className="text-2xl font-serif text-[#C47353] font-normal">¥{pkg.priceDisplay}</p>
                    <p className="text-xs text-[#8C7B6E]">元</p>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="mt-6">
          <Button
            ...
            className={`w-full py-6 text-lg font-serif rounded-full h-auto transition-all duration-250 ${
              selectedPackage && !loading
                ? 'bg-[#C47353] hover:bg-[#A85D40] text-white shadow-[0_4px_16px_rgba(196,115,83,0.3)] hover:-translate-y-[1px] active:scale-[0.98]'
        ...
        {/* 支付方式说明 */}
        <p className="text-center text-sm text-[#8C7B6E] mt-4 font-serif">
          点击支付后将生成微信支付二维码，请使用微信扫码支付
        </p>
      </div>
      <LawyerBottomNav />
    </div>
