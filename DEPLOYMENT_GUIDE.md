...
> 说明：项目会在 `scripts/build.sh` 中自动生成每次构建对应的 HTML cache-bust token，
> 让首页、登录页和管理页在 CDN 命中旧 HTML 后仍会刷新到当前构建的 chunk 文件。
> 如果你的域名前面还挂着独立 CDN，部署后建议同步执行一次缓存刷新。

### 2.1 线上静态资源 404 的快速判定

如果线上再次出现 `_chunk.js` 或媒体文件 404，按下面顺序判断：

1. 先打开页面源码，确认入口 HTML 是否带着本次构建对应的 `__bbwv` 参数。
   - 如果参数还是旧值，说明 CDN 或边缘缓存还在回放旧 HTML。
2. 再直接访问当前页面引用的 `/_next/static/...` chunk。
   - 如果 HTML 是新的，但 chunk 404，优先检查部署目录里是否真的同步了当前 `.next/static`。
3. 如果构建产物和部署目录都正确，但线上仍偶发 404。
   - 这通常就是独立 CDN 缓存未刷新，或者裸域名与 `www` 分别缓存了不同版本的 HTML。

一个很实用的判断方式是：

- HTML 新、chunk 旧：缓存问题
- HTML 新、chunk 不存在：部署同步问题
- HTML 旧、chunk 也旧：入口页还没切到新构建

### 2.2 根因矩阵

| 现象 | 最可能根因 | 你会看到的证据 | 修复动作 |
|---|---|---|---|
| 页面源码里还是旧 `__bbwv`，控制台一直 404 老 chunk | CDN / 边缘缓存回放旧 HTML | 访问同一路径的 HTML，`_next/static` 引用版本落后于当前构建 | 刷新 CDN，确认裸域名和 `www` 都清缓存 |
| HTML 已经是新版本，但 chunk 仍 404 | 部署目录没有同步当前构建产物 | 服务器上 `/.next/static/chunks` 与页面引用的文件名不一致 | 重新构建并同步完整 `.next` / `public` / `dist` |
| HTML 和 chunk 都指向同一构建，但资源仍断断续续 404 | 域名或反代路径配置异常 | 只有某个域名或某个入口页出问题，另一个域名正常 | 检查反向代理、站点根目录、静态目录映射 |
| 媒体文件 404 但 chunk 正常 | `public/` 未同步或旧页面还在引用旧文件名 | 页面里的图片路径能在仓库里找到，但线上请求返回 404 | 重新发布 `public/`，清掉旧页面缓存 |

### 2.3 这次问题最符合哪一种

从当前仓库证据看，最符合的是“HTML / CDN 缓存继续命中旧版本”，因为：

- `next.config` 没有设置 `assetPrefix` 或 `basePath`
- 当前构建产物里 `/.next/static/chunks` 是完整的
- 页面资源路径主要是站内绝对路径，没有明显子路径前缀错误
- 现象是“壳子偶尔能出，但功能 chunk 大量 404”，这正是 HTML 版本和静态 chunk 版本错配的典型表现
